...
class InferenceAgent:
    ...
    def run_audio_inference(self, img_pil, aud_path, crop, seed, nfe, cfg_scale):
        ...
        # latent_token_encoder always returns 32D (generator depends on this)
        t_lat = self.renderer.latent_token_encoder(s_tensor)
        if isinstance(t_lat, tuple): t_lat = t_lat[0]
        data['ref_x'] = t_lat  # 32D passed to generator — unchanged
        torch.manual_seed(seed)
        sample = self.generator.sample(data, a_cfg_scale=cfg_scale, nfe=nfe, seed=seed)
        d_hat = []
        T = sample.shape[1]
        # Expand reference latent 32D → 64D for renderer adapt/decode path
        te_lat = self.renderer.expand_mot(t_lat)
        ta_r = self.renderer.adapt(te_lat, g_r)
        m_r = self.renderer.latent_token_decoder(ta_r)
        for t in range(T):
            # Generator output is 32D; expand before feeding to renderer
            te_c = self.renderer.expand_mot(sample[:, t, ...])
            ta_c = self.renderer.adapt(te_c, g_r)
    ...
    def run_video_inference(self, source_img_pil, driving_video_path, crop):
        ...
        t_r = self.renderer.mot_encode(s_tensor)    # 32D
        te_r = self.renderer.expand_mot(t_r)        # 64D
        ta_r = self.renderer.adapt(te_r, i_r)
        ...
        while True:
            ...
            t_c = self.renderer.mot_encode(d_tensor)  # 32D
            te_c = self.renderer.expand_mot(t_c)       # 64D
            ta_c = self.renderer.adapt(te_c, i_r)
