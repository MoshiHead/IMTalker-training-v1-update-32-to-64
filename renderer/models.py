...
class MotionExpander(nn.Module):
    """
    Expands the original 32D motion latent to 64D via residual concatenation.

    Architecture:
        z_64 = LayerNorm( cat([z_32, complement_net(z_32)]) )

    The original 32D latent is structurally preserved as the first 32 dims
    of the pre-norm vector, enabling clean latent comparison and PCA analysis.
    The complement branch learns the additional 32D representation.

    This module is inserted after MotionEncoder and before IdentidyAdaptive
    / MotionDecoder, so the generator and all motion .pt files remain at 32D.
    """
    def __init__(self, in_dim=32, out_dim=64):
        super().__init__()
        assert out_dim == 2 * in_dim, "out_dim must equal 2 * in_dim for residual concat"
        self.complement_net = nn.Sequential(
            EqualLinear(in_dim, in_dim),
            nn.LeakyReLU(0.2),
            EqualLinear(in_dim, in_dim),
        )
    ...
    def forward(self, x):
        # x: (B, in_dim=32)
        complement = self.complement_net(x)            # (B, 32)
        expanded = torch.cat([x, complement], dim=-1)  # (B, 64)
        return self.norm(expanded)                     # (B, 64)
...
class IMTRenderer(nn.Module):
    def __init__(self, args):
        ...
        # MotionEncoder stays at dm=32 — generator and motion .pt files depend on this
        self.latent_token_encoder = MotionEncoder(initial_channels=64, output_channels=[128, 256, 512, 512, 512])
        # Expansion module: 32D latent → 64D expanded representation
        self.motion_expander = MotionExpander(in_dim=32, out_dim=64)
        # Downstream modules now operate in the expanded 64D space
        self.latent_token_decoder = MotionDecoder(latent_dim=64, const_dim=32)
        self.frame_decoder = SynthesisNetwork(args, self.feature_dims, self.spatial_dims)
        self.adapt = IdentidyAdaptive(dim_mot=64, dim_app=512)
    ...
    def mot_encode(self, x):
        """Returns the original 32D motion latent from MotionEncoder.
        Use expand_mot() afterwards if 64D is needed for adapt/decode."""
        mot_latent = self.latent_token_encoder(x)
        return mot_latent

    def expand_mot(self, z):
        """Expands a 32D motion latent to the 64D representation.
        Accepts tensors of shape (B, 32) or (B, T, 32) — handles sequence dims."""
        if z.dim() == 3:
            B, T, D = z.shape
            return self.motion_expander(z.reshape(B * T, D)).reshape(B, T, -1)
        return self.motion_expander(z)

    def mot_decode(self, x):
        """Decodes a 64D adapted latent into motion maps."""
        mot_map = self.latent_token_decoder(x)
        return mot_map

    def id_adapt(self, t, id):
        """t must be 64D (already expanded). Call expand_mot(z_32) first."""
        return self.adapt(t, id)

    def forward(self, x_current, x_reference):
        f_r, i_r = self.app_encode(x_reference)
        t_r = self.mot_encode(x_reference)   # 32D
        t_c = self.mot_encode(x_current)     # 32D
        te_r = self.expand_mot(t_r)          # 64D
        te_c = self.expand_mot(t_c)          # 64D
        ta_r = self.adapt(te_r, i_r)         # 64D → 64D
        ta_c = self.adapt(te_c, i_r)         # 64D → 64D
        ma_r = self.mot_decode(ta_r)
        ma_c = self.mot_decode(ta_c)
        output_frame = self.decode(ma_c, ma_r, f_r)
        return output_frame, t_c  # return original 32D for external callers