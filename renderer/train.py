...
class IMFSystem(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        t_0 = self.gen.mot_encode(ref)    # 32D original latent
        t_1 = self.gen.mot_encode(real)   # 32D original latent

        # Expand 32D → 64D before passing to identity-adaptive and decoder
        te_0 = self.gen.expand_mot(t_0)   # 64D
        te_1 = self.gen.expand_mot(t_1)   # 64D

        ta_10 = self.gen.id_adapt(te_1, id_0)  # 64D → 64D
        ta_11 = self.gen.id_adapt(te_1, id_1)  # 64D → 64D
        ta_12 = self.gen.id_adapt(te_1, id_2)  # 64D → 64D
        ta_00 = self.gen.id_adapt(te_0, id_0)  # 64D → 64D
        ...
        # Canonical/Distance Loss — computed in the 64D expanded space
        dist1 = torch.norm(te_1 - ta_11, dim=1)
        dist2 = torch.norm(te_1 - ta_12, dim=1)
